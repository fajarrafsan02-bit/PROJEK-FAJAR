// best-selling-products-component.js
class BestSellingProductsComponent {
    ...
    async init() {
        if (!this.container) {
            console.warn('BestSellingProductsComponent: Container element not found');
            return;
        }
    ...
    async loadBestSellingProducts() {
        ...
        try {
            const response = await fetch('/public/api/best-selling/top?limit=5');
            ...
            console.error('BestSellingProductsComponent: Error loading best selling products:', error);
        ...
        if (!products || products.length === 0) {
            this.container.innerHTML = `
                <div class="best-selling-products-no-data">
                    ...
                    <p>Produk terlaris akan muncul setelah ada transaksi penjualan.</p>
                </div>
            `;
            return;
        }
            ...
            return `
                <div class="best-selling-product-card fade-in-up stagger-animation" style="animation-delay: ${index * 0.1}s;">
                    ...
                    <div class="product-image">
                        <img src="${product.productImage || '/images/default-product.jpg'}" 
                    ...
                    <button class="add-to-cart-btn" 
                            ...
                            onclick="bestSellingProductsComponent.addToCart(this)">
    ...
    getRankStyles(rank) {
        switch (rank) {
            case 1:
                return {
                    style: 'style="background: linear-gradient(135deg, #ffd700 0%, #ffed4e 100%); color: #000;"',
                    icon: 'fas fa-medal'
                };
            case 2:
                return {
                    style: 'style="background: linear-gradient(135deg, #c0c0c0 0%, #e6e6e6 100%); color: #000;"',
                    icon: 'fas fa-award'
                };
            case 3:
                return {
                    style: 'style="background: linear-gradient(135deg, #cd7f32 0%, #e0bb9d 100%); color: #fff;"',
                    icon: 'fas fa-trophy'
                };
            default:
                return {
                    style: 'style="background: #f0f0f0; color: #333;"',
                    icon: ''
                };
        }
    }
    ...
    async addToCart(button) {
        ...
        try {
            ...
            console.error('BestSellingProductsComponent: Error adding product to cart:', error);
    ...
    async getCurrentUserId() {
        try {
            ...
            console.error('BestSellingProductsComponent: Error getting current user:', error);
        }
        return null;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.bestSellingProductsComponent = new BestSellingProductsComponent();
});